...
更新时间：2026-09-16（第六轮：FIX-016，缺陷清单 16 条全部实施）

当前分支：`main`

当前 `HEAD`：`57a4ab8`（`fix: stop offering a model swap for a commit the card refused`）。工作树干净。

## 交接结论

### 本轮（全面审查 + 批次 1—4 + FIX-016）

对项目做过一次全面审查（恶性问题 + 游玩期提示词两条线），共立案 16 条缺陷与 21 条提示词条目，产物是被忽略的 `local-development-records/` 下两份清单：

- `PROJECT-REVIEW-FIX-CHECKLIST-2026-09-16.md`：缺陷清单，**16 条全部已实施**
...
**FIX-016 是 FIX-004 判据的收尾**：模型答完之后、节点末数据提交被**卡或模块自己的声明**拒绝（权限、未声明的动作、未允许 best-effort、缺处理器、处理器返回形状错）时，原先仍提示"换模型重试"。这五个码现判为确定性。线画在"谁能修好"上：被**状态**（版本冲突、重复/缺失记录）或**模型自身产出**（schema 不合规、提交阶段故障）拒绝的提交仍走模型路径，因为 agent 节点重试会重新生成批次，自动重试有实际价值；`data_schema_invalid` 是**刻意**留在模型路径的（违规数据就是模型的产出）。②③ 的**面板措辞**仍不准，作为独立候选留在缺陷清单的待定稿里。

**尚未实施**：缺陷清单待定稿里的「选项 B」（把数据提交失败的措辞从模型下拉框改成普通重试）；两份清单各自登记的待定稿/待取证条目。**至此本轮立案的缺陷与提示词改动全部落地**；剩余工作不含服务端/运行时结构改动。

> **订正（2026-09-16）**：本文件此前三处写成"提示词清单 A 组 7 条 + C 组 1 条尚未实施"，**该说法错误**。提示词清单的索引与各条状态字段一直标记为已实施（`8382e74` / `485082a`），本次逐条比对磁盘原文复核，22 项断言全部通过。教训已记入提示词清单的「状态列是唯一权威」一节：交接摘要必须回读清单索引，不得凭记忆转述。
...
**已实施**：批次 1 全部；批次 2 的 FIX-003 / 004 / 011；批次 3 的 FIX-006b / 007；批次 4 的 FIX-008 / 009 / 010；FIX-015（批次 1 引入的校验器回归）；FIX-016（提交被卡声明拒绝时的归类）。**缺陷清单内的代码修复已全部落地。**

**提示词清单**：21 条**无待办**——A 组 7 条、B 组 P-104、C 组 1 条已实施；B 组其余三条与 D/E 组是"保留 / 明确不做"的决策记录；文末「待取证」是**尚未逐行读过的位置清单**，需要时才去读，不是待办队列。

**待实施**：

- **缺陷清单待定稿里的「选项 B」**：把节点失败码暴露给前端，让被状态/产出拒绝的数据提交显示"重试（会重新生成改动）"而不是模型下拉框。行为已正确，纯措辞改进，需用户决定是否值得动协议字段。
- 两份清单各自的其余「后续待定稿」与「待取证」条目；其中来自并行代理而未经逐行复核的，升级前必须先复核。
