...
**尚未实施**：提示词清单的 A 组 7 条 + C 组 1 条；两份清单各自登记的待定稿/待取证条目（其中一条是上面那个"措辞 vs 行为"的选项 B）。**至此本轮立案的代码缺陷已全部修复**；剩余工作不含服务端/运行时结构改动（提示词与文档为主）。
...
- **提示词清单**：A 组 7 条措辞改动 + C 组 1 条正向授权，全部已定稿可实施；其中 P-005 与已搬迁的 `prepare-recent-narrative-stories.mjs` 同文件。这是本轮剩余的**主要**工作，全部落在提示词与文档，不含运行时结构改动。
