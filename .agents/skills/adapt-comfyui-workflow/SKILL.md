...
3. Present detected prompt, seed, loader, LoRA, output, dimension, and custom-node candidates. Ask only about genuine ambiguities and the desired fixed positive/negative/style values. For the bound seed node, report the integer range it accepts — read it from the node's own metadata (`/object_info`, a widget's `max`) or from the custom node's documented limit — and say when you had to state it explicitly instead of reading it.
4. Propose the exact profile mapping, including `seedRange`, and the model guide. Do not write either until the user confirms the proposal.
...
6. Run `node scripts/validate-profile.mjs <profile-directory>`. Resolve every error before reporting completion. A missing or unrepresentable `seedRange` is an error: an out-of-range seed is accepted by ComfyUI, skips the save branch, and is still reported as a successful execution, so the failure would only appear as a missing image.
