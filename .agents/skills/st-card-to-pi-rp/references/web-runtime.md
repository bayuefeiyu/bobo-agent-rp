...
- `POST /api/workflow-runs/<id>/nodes/<id>/retry`, `POST .../cancel`, `POST .../skip`: explicit failure and blocking-run recovery. `POST .../nodes/<id>/process-record/open` opens a completed node's user-only process record.
