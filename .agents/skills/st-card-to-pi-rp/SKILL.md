...
3. what will remain fixed every turn;
4. how world, character, rule, and opening content will be split or kept on demand;
...
6. every available project-global module, its relevance or conflict, and the card-specific adaptation it would require;
...
State that original wording will be preserved primarily through splitting, sorting, and regrouping. When valid global modules exist, explicitly ask which ones to import; do not silently select them, and do not treat a general instruction to begin as a module selection unless the user explicitly delegates the choice. Recommend suitable global workflows, then ask whether the user wants to inspect/add other workflows or adjust the recommended nodes; a general confirmation accepts the stated workflow recommendation. If a catalog is empty, state that discovery found none without asking an empty question. End with a small numbered list of decisions that would benefit from discussion, and offer to discuss any or all of them one by one. Unless the user already gave explicit advance confirmation to start directly, end the turn after the proposal and wait without writing output files.

When the source has persistent variables, include the native variable inspector in the proposal by default even if the card has no authored status bar. It is the variable storage module's plain frontend view of the complete effective state, not a second data owner. State that the user may cancel this debugging surface; cancellation makes the variable module background-only without removing variable storage or updates. An authored status bar remains a separate source-derived frontend module and never replaces the inspector.
...
5. Build the card pack described in [target-card-pack.md](references/target-card-pack.md). Import only the project-global modules selected or explicitly delegated during the proposal, copying each into the card rather than linking to the global package; follow [global-modules.md](references/global-modules.md) for collision and adaptation rules. Read [workflow-system.md](references/workflow-system.md), copy every confirmed global workflow into the card, and apply only the confirmed card-specific node/trigger changes. Node models default to `pi:current`; reference saved Agent/model IDs rather than duplicating their configuration. Keep foundational world facts, the story premise, primary-character material, core card-specific writing rules, and a compact knowledge map in the fixed context. Move domain details, entity dossiers, supporting characters, and conditional scene rules into discoverable on-demand files. Convert every source-authored output outside the main narrative into a frontend feature module; do not leave its generated content appended to chat prose or reduce it to an on-demand rule file. Create the card's message retrieval policy. When source EJS or embedded prompt code exists, read [ejs-conversion.md](references/ejs-conversion.md), classify each behavior, and convert deterministic prompt selection to native context processors without executing the source. When the source contains persistent structured state, a card-specific function, or an auxiliary output channel, read [feature-modules.md](references/feature-modules.md) and convert it into a version 3 module with storage protocol version 2, common records, its own skill, and an explicit retrieval policy. For source variables, also read [variables.md](references/variables.md) and build a native full-snapshot variable module whose default frontend view is the complete generic variable inspector; only make it background when the user explicitly cancelled that surface. Choose Agent `contextOrder` from instruction dependencies and intended model behavior; choose `displayOrder` independently for presentation. Use a background module only when a non-output function needs the same data and prompt lifecycle but no visible Web panel; authored auxiliary outputs are always frontend modules.
...
- Every auxiliary output is a card-specific frontend feature module. Move its authored activation, content, and formatting instructions into that module's skill and use the post-narrative output engine; do not emit the auxiliary content in the chat body.
...
- A frontend native variable module is always the generic debugging inspector: its `view.json` contains the complete `snapshot.data.state` as a recursive JSON region. Do not replace it with an authored status bar or a hand-picked dashboard. Put those in separate modules. Only an explicit user cancellation may make the variable module background-only.
