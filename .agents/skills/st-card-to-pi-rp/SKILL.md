...
Handle character identity and setting fields, scenario, first and alternate greetings, dialogue examples, plain-text lorebooks, and persistent structured records that can be represented by the unified Pi RP data protocol. Resolve `{{char}}`, `<char>`, and `<bot>` to their definite referent during conversion, recording the basis when the card title differs from that character. Preserve the canonical `{{user}}` in authored text and prompts; normalize source `<user>` to it. Runtime binds `{{user}}` to the selected player name when play begins. Never put a player macro in IDs, paths, keys, or other structural fields. Preserve original source quotations in provenance unchanged.
...
Give each listed module a letter (`A`, `B`, `C` and so on) alongside its stable ID, place a recommended module first when there is one, and say that the user may select multiple letters. A letter is only a reply shortcut for this list; record the selected module IDs as the actual decision.
...
Read every selected module's `dependencies.json` before sending that message. An `orchestratedBy` entry means the module is driven from outside and is not a standalone feature: say so in the preselection list, and if the user selects the module without its orchestrator, present the two paths from [global-modules.md](references/global-modules.md) — add the orchestrator, or decouple inside the card — with the disclosed workload and risk, and record the choice. Do not drop the integration silently and do not refuse the selection.
...
Every conversion proposal must also explain the play prompt structure: `prompts/` is copied into the card for manual editing, role-marker lines become real system/user/assistant messages, and adjacent same-role messages are merged. Preview the final role sequence and call out every adjacent same-role boundary and its merged result; for risky boundaries explain the concrete consequence and help the user adjust. Respect an informed choice to retain a risky design. Explain both tail modes on every conversion: `on-start` is the default, while `every-call` reapplies the tail after tools, and state which one this card will use. If the sequence ends in assistant, ask whether prefill is intended and explain provider incompatibility, possible disruption of file reading/tool calls, and repeated-tail effects. Confirmation of intent does not prove model support; do not silently change the role or insert a fake user message. After conversion, direct manual prompt edits are the creator's responsibility and do not trigger repeated approval prompts.
...
For any proposal item with several paths, give the item a number and label the paths `A`, `B`, `C` and so on. Put a recommended path at `A` when practical and explain why; do not force a recommendation where none is justified. Mark paths as single-choice or combinable and accept compact replies such as `2B+C`. Keep recorded choices stable and show new labels if a choice set changes. Do not create extra questions merely to apply this format.
...
7. Generate `provenance.json`, `unresolved.md`, and `conversion-report.md`. Every non-empty source unit must be mapped, explicitly archived as metadata, marked unsupported, or marked unresolved. When the card is built on the shipped foreground templates, also write `manifest.design_invariants` so validation holds it to the design it actually keeps, and record every remaining `design:` warning there with the reason the card differs on purpose.
