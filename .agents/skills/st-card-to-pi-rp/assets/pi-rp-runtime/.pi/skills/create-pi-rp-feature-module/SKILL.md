...
4. Use storage `engine: null` for ordinary modules. For a native variable module, read the variable-engine section in the schema reference, use hybrid full-snapshot history, preserve the source variable semantics rather than legacy MVU output syntax, and design exact named/path references.
...
Run `.agents/skills/st-card-to-pi-rp/scripts/validate_card_pack.py cards/<card-id>`. Confirm record-envelope validity, retrieval mode, skill routing, Web display, and suffix-delete cascade behavior.
